...
public class ValidationService {
    public static final String CANNOT_MODIFY_APPLICATION_ERROR =
            ...
            """;
    public static final String CANNOT_UPDATE_APPLICATION_STATUS = "Cannot update case in status of %s";
    ...
    public void checkUpliftFieldPermissions(final UserSummaryDTO userSummaryDTO) {
        boolean userCanEditUpliftApplied =
                this.isUserAuthorisedToEditField(userSummaryDTO, RestrictedField.UPLIFT_APPLIED);

        if (!userCanEditUpliftApplied
                || !this.isUserAuthorisedToEditField(userSummaryDTO, RestrictedField.UPLIFT_REMOVED)) {
    ...
    public boolean isUserActionValid(UserActionDTO request, UserSummaryDTO userSummaryDTO) {
        ...
        if (request.getAction() != null
                && (userSummaryDTO.getRoleActions() == null
                        || !userSummaryDTO
                                .getRoleActions()
                                .contains(request.getAction().getCode()))) {
        ...
        if (request.getNewWorkReason() != null
                && (userSummaryDTO.getNewWorkReasons() == null
                        || !userSummaryDTO
                                .getNewWorkReasons()
                                .contains(request.getNewWorkReason().getCode()))) {
            crimeValidationExceptionList.add(USER_DOES_NOT_HAVE_A_VALID_NEW_WORK_REASON_CODE);
        }

        if (request.getSessionId() != null
                && userSummaryDTO.getReservationsDTO() != null
                && !request.getSessionId()
                        .equalsIgnoreCase(userSummaryDTO.getReservationsDTO().getUserSession())) {
    ...
    public boolean isUserAuthorisedToEditField(UserSummaryDTO userSummaryDTO, RestrictedField restrictedField) {
        ...
        return userSummaryDTO.getRoleDataItem().stream()
                .anyMatch(i -> i.getDataItem().equals(restrictedField.getField())
    ...
    private void validateApplicationStatus(WorkflowRequest request, RepOrderDTO repOrderDTO) {
        ...
        boolean isUpdateAllowedOnRepOrder =
                (null == rorsStatus) || RepOrderStatus.getFrom(rorsStatus).isUpdateAllowed();
    ...
    private void validateApplicationTimestamp(WorkflowRequest request, RepOrderDTO repOrderDTO) {
        ...
        if (!applicationTimestamp
                ...
                .isEqual(repOrderTimestamp.truncatedTo(ChronoUnit.SECONDS))) {
