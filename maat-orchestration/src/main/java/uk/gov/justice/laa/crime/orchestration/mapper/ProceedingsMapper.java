...
public class ProceedingsMapper extends CrownCourtMapper {
    ...
    private void mapToApiUpdateApplicationRequest(
            ApplicationDTO application, UserDTO userDTO, ApiUpdateApplicationRequest request) {

        request.withRepId(NumberUtils.toInteger(application.getRepId()))
                ...
                .withDecisionReason(
                        DecisionReason.getFrom(application.getRepOrderDecision().getCode()))
        ...
        ApiCrownCourtSummary ccpCrownCourtSummary = crownCourtSummaryDtoToApiCrownCourtSummary(crownCourtSummary);

        request.withCrownCourtSummary(ccpCrownCourtSummary)
                .withApplicantHistoryId(
                        NumberUtils.toInteger(application.getApplicantDTO().getApplicantHistoryId()))
    ...
    private ApiCrownCourtSummary crownCourtSummaryDtoToApiCrownCourtSummary(CrownCourtSummaryDTO crownCourtSummary) {
        ...
        ccpCrownCourtSummary.setRepOrderDecision(
                crownCourtSummary.getRepOrderDecision().getValue());
        ...
        if (null != crownCourtSummary.getEvidenceProvisionFee()) {
            ccpCrownCourtSummary.setEvidenceFeeLevel(EvidenceFeeLevel.getFrom(
                    crownCourtSummary.getEvidenceProvisionFee().getFeeLevel()));
        }
    ...
    private void mapCrownCourtSummaryToApplication(
            ApiUpdateCrownCourtOutcomeResponse response, ApplicationDTO applicationDTO) {

        CrownCourtSummaryDTO crownCourtSummary =
                applicationDTO.getCrownCourtOverviewDTO().getCrownCourtSummaryDTO();
        crownCourtSummary.setCcRepOrderDate(
                DateUtil.toDate(response.getCrownCourtSummary().getRepOrderDate()));
        crownCourtSummary.setRepOrderDecision(
                new SysGenString(response.getCrownCourtSummary().getRepOrderDecision()));
        crownCourtSummary.setCcRepType(
                new SysGenString(response.getCrownCourtSummary().getRepType()));
        ...
        List<OutcomeDTO> outcomeDTOList = response.getCrownCourtSummary().getRepOrderCrownCourtOutcome().stream()
                .map(x -> {
    ...
    private ApiFinancialAssessment applicationDtoToFinancialAssessment(
        ...
        assessment.withInitStatus(
                getCurrentStatus(initialAssessment.getAssessmnentStatusDTO().getStatus()));

        if (financialAssessmentDTO.getFull().getAssessmentDate() != null) {
            assessment
                    .withFullResult(fullAssessment.getResult())
                    .withFullStatus(CurrentStatus.getFrom(
                            fullAssessment.getAssessmnentStatusDTO().getStatus()));
        }

        HardshipReviewDTO hardship;
        if (courtType == CourtType.CROWN_COURT) {
            hardship = financialAssessmentDTO.getHardship().getCrownCourtHardship();
        } else {
            hardship = financialAssessmentDTO.getHardship().getMagCourtHardship();
        }
    ...
    public ApplicationDTO updateApplicationResponseToApplicationDto(
            ApiUpdateApplicationResponse response, ApplicationDTO application) {
    ...
    public ApplicationDTO updateCrownCourtResponseToApplicationDto(
            ApiUpdateCrownCourtOutcomeResponse response, ApplicationDTO application) {
    ...
    private void mapEvidenceDetailsToRequest(ApiUpdateCrownCourtRequest request, ApplicationDTO application) {
        ...
        if (null != financialAssessmentDTO && null != financialAssessmentDTO.getIncomeEvidence()) {
            ...
            request.setIncomeEvidenceReceivedDate(
                    DateUtil.toLocalDateTime(incomeEvidenceSummaryDTO.getEvidenceReceivedDate()));
        }

        if (null != capitalEquityDTO && null != capitalEquityDTO.getCapitalEvidenceSummary()) {
            request.setCapitalEvidenceReceivedDate(DateUtil.toLocalDateTime(
                    capitalEquityDTO.getCapitalEvidenceSummary().getEvidenceReceivedDate()));
        }

        if (null != application.getCapitalEquityDTO()
                && null != application.getCapitalEquityDTO().getCapitalOther()) {

            List<ApiCapitalEvidence> apiCapitalEvidenceList =
                    application.getCapitalEquityDTO().getCapitalOther().stream()
                            .map(CapitalOtherDTO::getCapitalEvidence)
                            .flatMap(Collection::stream)
                            .map(x -> {
                                ApiCapitalEvidence apiCapitalEvidence = new ApiCapitalEvidence();
                                apiCapitalEvidence.setEvidenceType(
                                        x.getEvidenceTypeDTO().getEvidence());
