...
class PassportAssessmentIntegrationTest extends WiremockIntegrationTest {
    ...
    void givenValidLegacyId_whenFindIsInvoked_thenPassportAssessmentIsReturned() throws Exception {
        ...
        mvc.perform(buildRequest(HttpMethod.GET, ENDPOINT_URL + "/" + PASSPORT_ASSESSMENT_ID))
            ...
            .andExpect(jsonPath("$.passportId").value(expected.getPassportedId()))
            .andExpect(jsonPath("$.cmuId").value(expected.getCmuId()))
            .andExpect(jsonPath("$.usn").value(expected.getUsn()))
            .andExpect(jsonPath("$.date").value(expected.getDate()))
            .andExpect(jsonPath("$.assessmentStatusDTO").value(expected.getAssessementStatusDTO()))
            .andExpect(
                jsonPath("$.passportConfirmationDTO").value(expected.getPassportConfirmationDTO()))
            ...
            .andExpect(jsonPath("$.benefitGaurenteedStatePension").value(
                expected.getBenefitGaurenteedStatePension()))
            .andExpect(
                jsonPath("$.benefitClaimedByPartner").value(expected.getBenefitClaimedByPartner()))
            .andExpect(jsonPath("$.benefitEmploymentSupport").value(
                expected.getBenefitEmploymentSupport()))
            .andExpect(
                jsonPath("$.benefitUniversalCredit").value(expected.getBenefitUniversalCredit()))
            ...
            .andExpect(
                jsonPath("$.under18HeardYouthCourt").value(expected.getUnder18HeardYouthCourt()))
            .andExpect(
                jsonPath("$.under18HeardMagsCourt").value(expected.getUnder18HeardMagsCourt()))
            ...
            .andExpect(jsonPath("$.passportSummaryEvidenceDTO").value(
                expected.getPassportSummaryEvidenceDTO()))
    ...
    void givenEmptyAssessmentResponse_whenFindIsInvoked_thenFailsNotFound() throws Exception {
        stubForOAuth();
        wiremock.stubFor(get(urlMatching("/api/internal/v1/passport/lookup-by-legacy-id/"
            + Constants.PASSPORT_ASSESSMENT_ID)).willReturn(WireMock.notFound()));
