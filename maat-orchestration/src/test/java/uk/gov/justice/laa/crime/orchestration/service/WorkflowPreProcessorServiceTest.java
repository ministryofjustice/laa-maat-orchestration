...
class WorkflowPreProcessorServiceTest {
    ...
    @Test
    void givenUserIsNotAuthorised_whenWorkflowPreProcessorServiceIsInvoked_thenExceptionIsThrown() {
        WorkflowRequest request = getWorkflowRequestWithCaseType(INDICTABLE.getCaseType());
        RepOrderDTO repOrderDTO = TestModelDataBuilder.buildRepOrderDTO(RepOrderStatus.CURR.getCode());
        UserActionDTO userActionDTO = TestModelDataBuilder.getUserActionDTO();
        UserSummaryDTO userSummaryDTO = TestModelDataBuilder.getUserSummaryDTO();
        List<String> validationErrors = List.of(ValidationService.USER_DOES_NOT_HAVE_A_VALID_NEW_WORK_REASON_CODE);

        when(maatCourtDataService.getUserSummary(userActionDTO.getUsername())).thenReturn(userSummaryDTO);
        doThrow(new CrimeValidationException(validationErrors))
                .when(validationService)
    ...
    @Test
    void givenApplicationStatusChanged_whenWorkflowPreProcessorServiceIsInvoked_thenUpdatedWorkflowRequestIsReturned() {
        WorkflowRequest request = getWorkflowRequestWithCaseType(INDICTABLE.getCaseType());
        RepOrderDTO repOrderDTO = TestModelDataBuilder.buildRepOrderDTO(RepOrderStatus.CURR.getCode());
        UserActionDTO userActionDTO = TestModelDataBuilder.getUserActionDTO();
        UserSummaryDTO userSummaryDTO = TestModelDataBuilder.getUserSummaryDTO();
    ...
    void givenApplicationStatusHasNotChanged_whenWorkflowPreProcessorServiceIsInvoked_thenWorkflowRequestIsReturned(
            WorkflowRequest request, RepOrderDTO repOrderDTO) {
        UserActionDTO userActionDTO = TestModelDataBuilder.getUserActionDTO();
        UserSummaryDTO userSummaryDTO = TestModelDataBuilder.getUserSummaryDTO();
    ...
    private static Stream<Arguments> workflowRequestWithNoApplicationStatusChange() {
        return Stream.of(
                ...
                Arguments.of(TestModelDataBuilder.buildWorkFlowRequest(), TestModelDataBuilder.buildRepOrderDTO(null)),
                Arguments.of(
                        TestModelDataBuilder.buildWorkFlowRequest(),
                ...
                Arguments.of(
                        WorkflowRequest.builder()
                                ...
                                .build(),
                        TestModelDataBuilder.buildRepOrderDTO("RepStatus")),
                Arguments.of(getWorkflowRequestWithCaseType(INDICTABLE.getCaseType()), null),
                Arguments.of(
    ...
    void givenRequestValidationFails_whenPreProcessEvidenceRequestIsInvoked_thenExceptionIsThrown() {
        ...
        doThrow(new CrimeValidationException(
                        List.of(ValidationService.USER_DOES_NOT_HAVE_A_ROLE_CAPABLE_OF_PERFORMING_THIS_ACTION)))
                .when(validationService)
                .isUserActionValid(userActionDTO, userSummaryDTO);
    ...
    void givenUserNotAllowedToApplyUplift_whenPreProcessEvidenceRequestIsInvoked_thenExceptionIsThrown() {
        ...
        doThrow(new ValidationException(ValidationService.USER_DOES_NOT_HAVE_A_ROLE_CAPABLE_OF_PERFORMING_THIS_ACTION))
                .when(validationService)
                .checkUpliftFieldPermissions(userSummaryDTO);
